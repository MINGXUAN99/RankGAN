...
class Trainer():
    def __init__(self, args, modelD, modelG, Encoder, criterion, prevD, prevG):
        ...
        self.target_fakeD = torch.zeros(self.batch_size, self.nranks-1).to(self.device)
        self.target_fakeG = torch.zeros(self.batch_size, self.nranks-1).to(self.device)
        ...
        'Fake': {'dtype': output_dtype, 'vtype': output_vtype, 'win': 'fake'},
        'Fakes_Current': {'dtype': output_dtype, 'vtype': output_vtype, 'win': 'Fakes_Current'},
    ...
    def setup_stage(self, stage, dataloader=None):
        ...
        self.optimizerD = self.initialize_optimizer(self.modelD[0], lr=self.lr_dis*np.exp(-stage), optim_method=self.optim_method)
        self.optimizerG = self.initialize_optimizer(self.modelG[0], lr=self.lr_gen*np.exp(-stage), optim_method=self.optim_method)
        ...
        while i < self.len_dataset:
            ...
            while j < d_iterations and i < self.len_dataset:
                ...
                if self.add_noise:
                    ...
                    out_fake = self.modelD[0](dis_input)
                else:
                    out_fake = self.modelD[0](fake)
                loss_fake = self.logits_loss(out_fake, self.target_fakeD[:batch_size])
                logits_fake = self.sigmoid(out_fake)
                acc_fake = 100*self.logits_eval(logits_fake, self.target_fakeD[:batch_size])
                ...
                if j < d_iterations:
                    ...
                    print('Stage %d: [%d/%d][%d/%d] Loss_D0: %.3f Loss_G0: %.3f Acc_Real %.1f Acc_Fake %.1f'
                        % (stage, epoch, self.stage_epochs[stage], i, self.len_dataset, net_error.item(), 0, acc_real, acc_fake))
                    ...
                    if i % self.plot_update_interval == 0:
                        ...
                        losses_train['Fake'] = fake.detach().cpu()
                        losses_train['Fakes_Previous'] = self.modelG[1](self.fixed_noise).detach().cpu()
                        losses_train['Fakes_Current'] = self.modelG[0](self.fixed_noise).detach().cpu()
            ...
            while j < self.g_iter and i < self.len_dataset:
                ...
                if self.add_noise:
                    ...
                    out_G = self.modelD[0](dis_input)
                else:
                    out_G = self.modelD[0](fake)

                loss_G = self.logits_loss(out_G, self.target_fakeG[:batch_size])
                # logits_G = self.sigmoid(out_G2)
                # Acc_Fake = self.logits_eval(out_G2, self.target_real[:batch_size])
                ...
                if j < self.g_iter:
                    ...
                    losses_train['Loss_G0'] = loss_G.item()
                    losses_train['Acc_Real'] = acc_real
                    losses_train['Acc_Fake'] = acc_fake
                    self.monitor_train.update(losses_train, batch_size)
                    print('Stage %d: [%d/%d][%d/%d] Loss_D0: %.3f Loss_G0: %.3f Acc_Real %.1f Acc_Fake %.1f'
                        % (stage, epoch, self.stage_epochs[stage], i, self.len_dataset, 0, loss_G2.item(), acc_real, acc_fake))
            ...
            self.monitor_train.update(losses_train, batch_size)
            print('Stage %d: [%d/%d][%d/%d] Loss_D0: %.3f Loss_G0: %.3f Acc_Real %.1f Acc_Fake %.1f'
                % (stage, epoch, self.stage_epochs[stage], i, self.len_dataset, net_error.item(), loss_G.item(), acc_real, acc_fake))
